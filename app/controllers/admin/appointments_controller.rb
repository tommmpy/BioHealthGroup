module Admin
  class AppointmentsController < Admin::BaseController
    ...
    def create
      ...
      if @appointment.save
        redirect_to admin_appointments_path, notice: "Turno agendado correctamente."
      else
        @pacientes = User.where(role: User.roles[:paciente]).order(:first_name)
        @medicos = User.where(role: User.roles[:medico]).order(:first_name)
        @branches = Branch.where(enabled: true).order(:name)
    ...
    def update
      ...
      if @appointment.update(appointment_params)
        redirect_to admin_appointment_path(@appointment), notice: "Turno actualizado correctamente."
      else
        @pacientes = User.where(role: User.roles[:paciente]).order(:first_name)
        @medicos = User.where(role: User.roles[:medico]).order(:first_name)
        @branches = Branch.where(enabled: true).order(:name)
    ...
    def destroy
      @appointment = Appointment.find(params[:id])
      @appointment.destroy
      redirect_to admin_appointments_path, status: :see_other, notice: "Turno eliminado correctamente."
    end

    def confirm
      @appointment = Appointment.find(params[:id])
      @appointment.update(status: :confirmed)
      redirect_to admin_appointment_path(@appointment), notice: "Turno confirmado."
    end

    def cancel
      @appointment = Appointment.find(params[:id])
      @appointment.update(status: :cancelled)
      redirect_to admin_appointments_path, notice: "Turno cancelado."
    end
    ...
    def appointment_params
      params.require(:appointment).permit(
        :user_id, :estudio_id, :medico_id, :branch_id,
