...
module Staff
  class EstudiosController < Staff::BaseController
      ...
      respond_to do |format|
        ...
        format.csv do
          ...
          csv_data = CSV.generate(headers: true) do |csv|
            ...
            @estudios.each do |e|
              csv << [
                e.id, e.nombre_completo, e.user&.ci, e.estado.humanize,
                e.fecha_estudio.strftime("%d/%m/%Y %H:%M"), e.branch&.name,
                e.medico.present? ? "#{e.medico.first_name} #{e.medico.last_name}" : "",
                e.cantidad_productos, Array(e.tipo_producto).join(", "),
    ...
    def create
      ...
      if @estudio.save
        redirect_to staff_estudios_path, notice: "Estudio agendado correctamente."
      else
    ...
    def update
      ...
      if @estudio.update(estudio_update_params)
        ...
        redirect_to staff_estudio_path(@estudio), notice: "Estudio actualizado correctamente."
      else
    ...
    def destroy
      @estudio = Estudio.find(params[:id])
      @estudio.destroy
      redirect_to staff_estudios_path, status: :see_other
    end

    def iniciar
      @estudio = Estudio.find(params[:id])
      Estudios::Workflow.call(@estudio, :iniciar, current_user: current_user)
      redirect_to staff_estudios_path
    end
    ...
    def finalizar
      ...
      if result[:success]
        redirect_to staff_estudios_path, notice: result[:notice]
      else
        redirect_to result[:alert] ? edit_staff_estudio_path(@estudio) : staff_estudios_path, alert: result[:alert]
      end
    end
    ...
    def descargar_informe
      ...
      pdf.number_pages "BioHealth Group - Documento oficial generado el #{Time.now.strftime('%d/%m/%Y %H:%M')} - Página <page> de <total>",
        at: [ pdf.bounds.left, -20 ], size: 8, color: "999999"
    ...
    def estudio_params
      params.require(:estudio).permit(:user_id, :nombre_completo, :cantidad_productos, :fecha_estudio, :branch_id, :medico_id, tipo_producto: []).tap do |whitelisted|
